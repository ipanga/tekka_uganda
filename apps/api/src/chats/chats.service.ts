...
export class ChatsService {
  ...
  async markAsRead(chatId: string, userId: string) {
    ...
    // Sweep MESSAGE-type notifications generated by this chat so the
    // notification badge and the chat badge stay in sync — reading the
    // conversation here is treated as reading the related notifications too.
    // Notifications link back to the chat via the `data` JSON column
    // (set in sendMessage at ~line 350): { chatId, messageId }.
