...
// Flat settings shape matching the actual backend API response
...
export default function SettingsPage() {
  ...
  const loadSettings = async () => {
    try {
      setLoading(true);
      // Backend returns flat settings object, cast accordingly
      const data = await api.getMySettings() as unknown as FlatSettings;
      setSettings(data);
    } catch (error) {
  ...
  const updateSetting = async (key: string, value: unknown) => {
    ...
    setSaving(true);
    try {
      const updated = await api.updateMySettings({ [key]: value } as never) as unknown as FlatSettings;
      setSettings(updated);
    } catch (error) {
          ...
          <Card className="mb-6">
            ...
            <CardContent className="space-y-4">
              ...
              <ToggleSetting
                ...
                description="Get notified about activity on your listings"
                checked={settings?.listingNotifications ?? true}
                onChange={(v) => updateSetting('listingNotifications', v)}
              />
            </CardContent>
          </Card>
